...
public class MzIdentmlToMzTab {
    ...
    private Map<String, SearchDatabase> searchDatabaseHashMap
            = new HashMap<>();
    ...
    public static void main(String args[]) {
        ...
        MzIdentmlToMzTab mzIdentMLToMzTab = new MzIdentmlToMzTab(args[0],
                                                                 args[1]);
    }
        ...
        try {
            ...
            try {
                ...
                String methodName = Thread.currentThread().getStackTrace()[1].
                        getMethodName();
                String className = this.getClass().getName();
                String message = "The task \"" + methodName
                        + "\" in the class \"" + className
                        + "\" was not completed because of " + ex.getMessage()
                        + "."
                        + "\nPlease see the reference guide at 02 for more information on this error. https://code.google.com/p/mzidentml-lib/wiki/CommonErrors ";
                System.out.println(message);
            }
    ...
    private String writeHeader() {
        ...
        header.append(
                "MTD\tdescription	mzTab example file for reporting a summary report of identification data\n");

//             MTD	ms_run[1]-location	file://C:/User/name/data/xyc_heat_1.mzML																
        SpectraData spectraData = mzIdentMLUnmarshaller.unmarshal(
                MzIdentMLElement.SpectraData);
        if (spectraData != null && spectraData.getLocation() != null) {
            header.append("MTD\tms_run[1]-location\t file://").append(
                    spectraData.getLocation().replace("\\", "/")).append("\n");
        ...
        for (int z = 0; z < spectrumIdentificationProtocolList.size(); z++) {
            SpectrumIdentificationProtocol spectrumIdentificationProtocol
                    = spectrumIdentificationProtocolList.get(z);
            ModificationParams modificationParams
                    = spectrumIdentificationProtocol.getModificationParams();
            ...
            for (int i = 0; i < searchModificationList.size(); i++) {
                ...
                for (int j = 0; j < cvParamList.size(); j++) {
                    ...
                    if (searchModification.isFixedMod()) {
                        ...
                        String newEntry = "MTD\tfixed_mod[" + fixed_mod_count
                                + "]\t[UNIMOD, " + cvParam.getAccession() + ", "
                                + cvParam.getName() + ", ]\n";
                        fixed_mod = fixed_mod + newEntry;
                        fixedMods.add(cvParam.getName());
                    } else {
                        variable_mod_count = variable_mod_count + 1;
                        String newEntry = "MTD\tvariable_mod["
                                + variable_mod_count + "]\t[UNIMOD, " + cvParam.
                                getAccession() + ", " + cvParam.getName()
                                + ", ]\n";
        ...
        if (analysisSoftwareList != null) {
            List<AnalysisSoftware> allASList = analysisSoftwareList.
                    getAnalysisSoftware();
            for (int i = 0; i < allASList.size(); i++) {
                AnalysisSoftware analysisSoftware = allASList.get(i);
                if (analysisSoftware.getSoftwareName() != null
                        && analysisSoftware.getSoftwareName().getCvParam()
                        != null) {
                    search_engine = analysisSoftware.getSoftwareName().
                            getCvParam().getAccession() + ","
                            + analysisSoftware.getSoftwareName().getCvParam().
                            getName() + ",";
                }
                break;
        ...
        SequenceCollection sequenceCollection = mzIdentMLUnmarshaller.unmarshal(
                MzIdentMLElement.SequenceCollection);
    ...
    private String writePRT() {
        ...
        prt.append(
                "PRH\taccession\tdescription\ttaxid\tspecies\tdatabase\tdatabase_version\tsearch_engine\tbest_search_engine_score\tsearch_engine_score_ms_run[1]\tambiguity_members\tmodifications\tprotein_coverage\n");
        try {
            String ambiguity_members = "";
            Iterator<MzIdentMLObject> iterProteinAmbiguityGroup
            ...
            while (iterProteinAmbiguityGroup.hasNext()) {
                ProteinAmbiguityGroup proteinAmbiguityGroup
                        = (ProteinAmbiguityGroup) iterProteinAmbiguityGroup.
                        next();
                ...
                for (int j = 0; j < proteinAmbiguityGroup.
                    ...
                    for (int s = 0; s < cvParamListproteinDetectionHypothesis.
                            size(); s++) {
                        CvParam cvParam = cvParamListproteinDetectionHypothesis.
                                get(s);
                ...
                for (int j = 0; j < proteinAmbiguityGroup.
                    ...
                    ProteinDetectionHypothesis proteinDetectionHypothesis
                            = proteinAmbiguityGroup.
                            getProteinDetectionHypothesis().get(j);
                    String dBSequenceRef = proteinDetectionHypothesis.
                            getDBSequenceRef();
                    if (!dBSequenceRef.equals(anchorProteinDetectionHypothesis.
                            getDBSequenceRef())) {
                        DBSequence dBSequence = mzIdentMLUnmarshaller.unmarshal(
                                DBSequence.class, dBSequenceRef);
                        ambiguity_members = ambiguity_members + dBSequence.
                                getAccession() + ",";
                    }

                }
                if (!ambiguity_members.equals("")) {
                    ambiguity_members = ambiguity_members.substring(0,
                                                                    ambiguity_members.
                                                                    length() - 1);
                } else {
                ...
                for (int i = 0; i < peptideHypothesisList.size(); i++) {
                    PeptideHypothesis peptideHypothesis = peptideHypothesisList.
                            get(i);
                    String peptideEvidenceRef = peptideHypothesis.
                            getPeptideEvidenceRef();
                    PeptideEvidence peptideEvidence = mzIdentMLUnmarshaller.
                            unmarshal(PeptideEvidence.class, peptideEvidenceRef);
                    ...
                    Peptide peptide = mzIdentMLUnmarshaller.unmarshal(
                            Peptide.class, peptideRef);
                    List<Modification> modificationsList = peptide.
                            getModification();
                        ...
                        for (int k = 0; k < cvParamList.size(); k++) {
                            ...
                            if (cvParam.getAccession().startsWith("UNIMOD")) {
                                if (!fixedMods.contains(cvParam.getName())) {
                                    ...
                                    modifications = modifications + pos + "-"
                ...
                if (!modifications.equals("")) {
                    modifications = modifications.substring(0, modifications.
                                                            length() - 1);
                }

                String dBSequenceRef = anchorProteinDetectionHypothesis.
                        getDBSequenceRef();
                DBSequence dBSequence = mzIdentMLUnmarshaller.unmarshal(
                        DBSequence.class, dBSequenceRef);
                ...
                SearchDatabase searchDatabase = searchDatabaseHashMap.get(
                        searchDatabaseRef);
    ...
    private String writePSM() {
        ...
        psm.append(
                "PSH\tsequence\tPSM_ID\taccession\tunique\tdatabase\tdatabase_version\tsearch_engine\tsearch_engine_score\tmodifications\tspectra_ref\tretention_time\tcharge\texp_mass_to_charge\tcalc_mass_to_charge\tpre\tpost\tstart\tend\n");
        int pSM_ID = 1;
        try {
            ...
            while (iterSpectrumIdentificationResult.hasNext()) {
                ...
                for (int t = 0; t < spectrumIdentificationItemList.size(); t++) {
                    SpectrumIdentificationItem spectrumIdentificationItem
                            = spectrumIdentificationItemList.get(t);
                    String peptideRef = spectrumIdentificationItem.
                            getPeptideRef();
                    Peptide peptide = mzIdentMLUnmarshaller.unmarshal(
                            Peptide.class, peptideRef);
                    ...
                    for (int i = 0; i < peptideEvidenceRefList.size(); i++) {
                        ...
                        Peptide peptide1 = mzIdentMLUnmarshaller.unmarshal(
                                Peptide.class, peptideRef1);
                        List<Modification> modificationsList = peptide1.
                                getModification();
                        for (int j = 0; j < modificationsList.size(); j++) {
                            Modification modification = modificationsList.get(j);
                            int location = modification.getLocation().intValue();
                            List<CvParam> cvParamList = modification.
                                    getCvParam();
                            for (int k = 0; k < cvParamList.size(); k++) {
                                CvParam cvParam = cvParamList.get(k);
                                if (cvParam.getAccession().startsWith("UNIMOD")) {
                    ...
                    if (!modifications.equals("")) {
                        modifications = modifications.substring(0,
                                                                modifications.
                                                                length() - 1);
                    }
                    search_engine_score = "";
                    ...
                    for (int i = 0; i < cvParamList.size(); i++) {
                        ...
                        search_engine_score = search_engine_score + cvParam.
                                getAccession() + "," + cvParam.getName() + ","
                                + cvParam.getValue();
                    ...
                    for (int i = 0; i < peptideEvidenceRefList.size(); i++) {
                        ...
                        DBSequence dBSequence = mzIdentMLUnmarshaller.unmarshal(
                                DBSequence.class, dbSeqRef);

                        psm.append(dBSequence.getAccession()).append("\t");//Accession
                        psm.append("null\t");//unique
                        String searchDatabaseName = null;
                        String searchDatabaseVersion = null;
                        String searchDatabaseRef = dBSequence.
                                getSearchDatabaseRef();
                        SearchDatabase searchDatabase = searchDatabaseHashMap.
                                get(searchDatabaseRef);
                        if (searchDatabase != null) {
                            Param param = searchDatabase.getDatabaseName();
                            if (param != null) {
                                UserParam userParam = param.getUserParam();
                        ...
                        psm.append("ms_run[1]:").append(
                                spectrumIdentificationResult.getSpectrumID()).
                                append("\t");//spectra ref

                        List<CvParam> cvParamListSir
                                = spectrumIdentificationResult.getCvParam();
                        String rt = null;
                        ...
                        psm.append(spectrumIdentificationItem.getChargeState()).
                                append("\t");//ChargeState
                        psm.append(spectrumIdentificationItem.
                                getCalculatedMassToCharge()).append("\t");//CalculatedMassToCharge
                        psm.append(spectrumIdentificationItem.
                                getExperimentalMassToCharge()).append("\t");//ExperimentalMassToCharge
